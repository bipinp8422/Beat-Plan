...
def load_from_supabase(table_name, columns):
    ...
    try:
        ...
        if response.data:
            ...
            # Convert date strings back to date objects for VisitDate
...
def save_to_supabase(table_name, df):
    """Save DataFrame to Supabase (delete old + insert new)"""
    try:
        df_copy = df.copy()
        
        # Remove 'id' column if exists (auto-generated by Supabase)
        if 'id' in df_copy.columns:
            df_copy = df_copy.drop('id', axis=1)
        
        # Convert date columns to strings
        for col in df_copy.columns:
            if hasattr(df_copy[col].dtype, 'name') and 'datetime' in df_copy[col].dtype.name:
                df_copy[col] = df_copy[col].astype(str)
            elif col == 'VisitDate' or col == 'created_at':
...
if "employee_df" not in st.session_state:
    st.session_state.employee_df = load_from_supabase(
        "employee_master", 
        ["EmployeeCode", "EmployeeName", "Password"]
    )
if "gst_df" not in st.session_state:
    st.session_state.gst_df = load_from_supabase(
        "gst_master", 
        ["StoreID", "StoreName", "GSTNumber", "City", "EmployeeCode"]
    )
if "planned_df" not in st.session_state:
    st.session_state.planned_df = load_from_supabase(
        "planned_visits", 
        ["EmployeeCode", "EmployeeName", "City", "Store", "GSTNumber", "StoreID", "VisitDate"]
    )
if "admin_df" not in st.session_state:
    st.session_state.admin_df = load_from_supabase(
        "admin_master", 
        ["Username", "Password"]
    )
...
if st.session_state.role == "admin":
    ...
    if admin_menu == "📊 Dashboard":
        ...
        with tab1:
            if not st.session_state.employee_df.empty:
        ...
        st.info("Click below to refresh all data from Supabase")
    ...
    if emp_menu == "🎯 New Beat Plan":
        ...
        if my_plans.empty:
            ...
            with col2:
                ...
                my_plans_copy = my_plans.copy()
                my_plans_copy["Month"] = pd.to_datetime(my_plans_copy["VisitDate"]).dt.to_period("M").astype(str)
                st.line_chart(my_plans_copy.groupby("Month").size())
